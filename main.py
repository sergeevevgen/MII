...
about_clusters = "Обучение без учителя - метод k-средних Это итеративный алгоритм кластеризации, основанный на " \
                 "минимизации " \
                 ...
                 "выбираются три элемента выборки, в соответствие которым ставятся три кластера, в каждый из которых " \
                 "теперь " \
                 "включено по одной точке, каждая при этом является центроидом этого кластера. Далее ищем ближайшего " \
                 "соседа " \
                 "текущего центроида. Добавляем точку к соответствующему кластеру и пересчитываем положение центроида " \
                 "с учетом" \
                 "координат новых точек.  Алгоритм заканчивает работу, когда координаты каждого центроида перестают " \
                 "меняться." \
                 "Центроид каждого кластера в результате представляет собой набор значений признаков, описывающих " \
                 "усредненные" \
...
@app.route("/fuzzyfication", methods=['GET'])
def fuzzyfication():
    data = request.args
    a = float(data['abcd'].split(';')[0])
    b = float(data['abcd'].split(';')[1])
    c = float(data['abcd'].split(';')[2])
    d = float(data['abcd'].split(';')[3])
    a1 = float(data['a1b1c1d1'].split(';')[0])
    b1 = float(data['a1b1c1d1'].split(';')[1])
    c1 = float(data['a1b1c1d1'].split(';')[2])
    d1 = float(data['a1b1c1d1'].split(';')[3])
    objects = list(map(float, data['objects'].split(';')))
    objects.extend(list(map(float, data['objects1'].split(';'))))
